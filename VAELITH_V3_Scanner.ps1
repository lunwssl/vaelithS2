...
$T1_BtnRun.Add_Click({
    ...
    Write-RTB $T1_RTB "System Check Complete — Proceed to Step 2." $C_Cyan

    $T1_StatusLbl.Text      = "Complete — System Check passed"
                ...
                if ($props) {
                    foreach ($prop in $props.PSObject.Properties) {
                        if ($prop.Name -notmatch "^PS" -and $prop.Value -is [byte[]] -and $prop.Value.Length -ge 8) {
                            # Parse FILETIME (8 bytes little-endian)
...
$T7_BtnLoad.Add_Click({
    ...
    # From Scheduled Task runs (Event Log)
    try {
...
# ─── TAB CHANGE EVENT (auto-load) ─────────────────────────────────────────────
$Tabs.Add_SelectedIndexChanged({
    $sel = $Tabs.SelectedTab
    if ($sel -eq $Tab5 -and $script:T5_AllRows.Count -eq 0) {
        Load-T5Processes
    }
})
